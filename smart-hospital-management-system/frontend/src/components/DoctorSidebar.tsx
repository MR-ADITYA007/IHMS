import { CalendarDays, User, LogOut, Activity, Menu, X, Stethoscope } from 'lucide-react';
...
interface DoctorSidebarProps {
  ...
  onSectionChange: (section: DoctorSection) => void;
}

const navItems = [
  { id: 'schedule' as DoctorSection, label: 'Schedule', icon: CalendarDays, description: 'Daily appointments' },
  { id: 'quickview' as DoctorSection, label: 'Patient Quick-View', icon: User, description: 'Patient records' },
];

export default function DoctorSidebar({ activeSection, onSectionChange }: DoctorSidebarProps) {
      ...
      <div className="px-4 py-5 border-b border-sidebar-border">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-sidebar-foreground/60 font-medium uppercase tracking-wide">Doctor Portal</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-b border-sidebar-border">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-full bg-sidebar-accent flex items-center justify-center">
            <Stethoscope className="w-4 h-4 text-sidebar-accent-foreground" />
          </div>
          <div>
            <p className="text-sm font-semibold text-sidebar-foreground">Dr. Sarah Chen</p>
            <p className="text-[10px] text-sidebar-foreground/60">Cardiologist</p>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`md:hidden fixed top-0 left-0 h-full w-64 z-40 bg-sidebar transition-transform duration-300 ${
